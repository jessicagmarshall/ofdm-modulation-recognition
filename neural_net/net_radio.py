...
from ops import *
from read_data_radio import dataSet, testSet
...
class modulation_net(object):
	...
	def build_net(self, batch_size, train=True):
		...
		conv1 = tf.nn.elu(conv2d(self.msg, 64, k_h=1, k_w=4, d_h=1, d_w=1, name='conv1'))

		conv1_padded = tf.pad(conv1, [[0, 0], [0, 0], [1, 1] , [0, 0]])
		conv2 = tf.nn.elu(conv2d(conv1_padded, 32, k_h=2, k_w=4, d_h=1, d_w=1, padding='VALID', name='conv2'))

		conv2_flat = tf.reshape(conv2,[batch_size,-1])

		dense1 = tf.nn.elu(linear(conv2_flat, 256, name='lin1'))
		dense2 = tf.nn.elu(linear(dense1, 128, name='lin2'))
		output = linear(dense2, self.nClasses, name='lin3')
	...
	def test(self):
		...
		cm = np.zeros((6,6))
		print 'testing'
		for pos in xrange(220000, 300000, 256*10):		#220000
			#print pos
		...
		#print 'percent correct ', np.sum(np.diagonal(cm)) / ((300000 - 260000)/2560), '%\n'
		print 'percent correct ', 100*np.sum(np.diagonal(cm)) / np.sum(cm), '%\n'
		print 'test error', sum(testloss)/len(testloss)
		plt.figure()
		plot_confusion_matrix(cm*nClasses/np.sum(cm), title='Confusion Matrix (received OFDM data)')
		plt.show()

def plot_confusion_matrix(cm, title='Confusion Matrix (received OFDM data)', cmap=plt.cm.Blues):
...
nSamples = 256 # number of samples per example
